...
const Station = () => {
  ...
  const { isPlaying, isLoading, currentStation, playStation, togglePlay } = useStationAudioPlayer();
  ...
  const handlePlayClick = useCallback(() => {
    ...
    console.log(
      `User clicked ${isStationPlaying ? 'pause' : 'play'} on ${station.name} from source: Station Details Page`
    );
  }, [isStationActive, isStationPlaying, station, togglePlay, playStation]);

  return (
    <div>
      <button onClick={() => router.history.back()} className="text-blue-400 hover:text-blue-300 mb-8 cursor-pointer">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-2/3">
          <h1 className="text-4xl lg:text-5xl font-extrabold text-white mb-2">{station.name}</h1>
          <p className="text-lg text-gray-300 mb-6">{station.description}</p>

          <div className="space-y-4 my-8">
            <div>
              <div className="flex justify-between items-center mb-1">
                <h4 className="text-sm font-bold text-gray-400 uppercase tracking-wider">Popularity</h4>
                <span className="text-sm font-bold text-yellow-400">{station.popularity ?? '-'} / 5</span>
              </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <h4 className="text-sm font-bold text-gray-400 uppercase tracking-wider">Reliability</h4>
                <span className="text-sm font-bold text-green-400">{station.reliability} / 100</span>
              </div>
              <div className="w-full bg-slate-700 rounded-full h-2.5">
                <div className="bg-green-500 h-2.5 rounded-full" style={{ width: `${station.reliability}%` }}></div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {station.tags.map((tag) => (
              <span key={tag} className="bg-slate-700 text-blue-300 text-xs font-semibold px-3 py-1 rounded-full">
                {tag}
              </span>
            ))}
          </div>
